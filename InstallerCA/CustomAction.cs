            ...
            try
            {
                ...
                if (szOfficeRegKeyVersions.Length > 0)
                {
                    ...
                    foreach (string szOfficeVersionKey in lstVersions)
                    {
                        ...
                        {
                            session.Log("Retrieving LocalMachine registry key for : " + szBaseAddInKey + szOfficeVersionKey);
                            if (Registry.LocalMachine.OpenSubKey(szBaseAddInKey + szOfficeVersionKey, false) != null)
                            {
                                session.Log("Create registry key for : " + szBaseAddInKey + szOfficeVersionKey + @"\Excel\Options");
                                Registry.CurrentUser.CreateSubKey(szBaseAddInKey + szOfficeVersionKey + @"\Excel\Options");
                            }
                        }
